    ...
    <td><h3 class="mw-admin-main-section-inner-panel-sub-title mw-text-upper"><?php _e('Shipping address'); ?></h3>
      ...
      <a target="_blank"
                   href="https://maps.google.com/maps?q=<?php print urlencode($map_click_str) ?>&safe=off"> <img class="map-shipping-address"
                        src="http://maps.googleapis.com/maps/api/staticmap?size=200x200&zoom=17&markers=icon:http://piksera.com/order.png|<?php print urlencode($map_click_str) ?>&sensor=true&center=<?php print urlencode($map_click_str) ?>" style="max-width:100%; float:left;"/> </a></td>
  </tr>
</table>
...
<table  cellspacing="0" cellpadding="0" class="mw-ui-table mw-ui-table-basic mw-ui-table-order-info" style="margin-top:0">
  ...
  <tr>
    ...
    <td valign="top"><a target="_blank"
                    ...
                    src="https://maps.googleapis.com/maps/api/staticmap?size=200x200&zoom=17&markers=icon:https://piksera.com/user.png|<?php print urlencode($ord['payment_country'] . ',' . $ord['payment_city'] . ',' . $ord['payment_address']); ?>&sensor=true&center=<?php print urlencode($ord['payment_country'] . ',' . $ord['payment_city'] . ',' . $ord['payment_address']); ?>"/> </a></td>
  </tr>
</table>
<?php }  ?>
