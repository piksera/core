...
mw.admin_import = {
    export : function(manifest){
	 ...
	 $.post(mw.settings.api_url+'Piksera/Utils/Import/export', manifest ,
	 	function(msg) {
	...
	restore_to_page : function(src,page_id){
		...
		$.post(mw.settings.api_url+'Piksera/Utils/Import/restore', data ,
			function(msg) {
				mw.reload_module('admin/import/manage');
	...
	move_uploaded_file_to_import : function(src){
		...
		data = {}
		data.src=src;
		$.post(mw.settings.api_url+'Piksera/Utils/Import/move_uploaded_file_to_import', data ,
			function(msg) {
				mw.reload_module('admin/import/manage');
				mw.notification.msg(msg, 5000);
			});
	...
	create_full : function(selector){
		...
		$.post(mw.settings.api_url+'Piksera/Utils/Import/create_full', false ,
			function(msg) {
				mw.reload_module('admin/import/manage');
	...
	remove : function($id, $selector_to_hide){
        ...
        mw.tools.confirm(mw.msg.del, function(){
      		...
      		$.post(mw.settings.api_url+'Piksera/Utils/Import/delete', data ,
      			function(resp) {
      			 //mw.reload_module('admin/import/manage');
	...
	start_batch_process : function(){
		...
		$.post(mw.settings.api_url+'Piksera/Utils/Import/batch_process', {process:true} ,
	...
	cancel_batch_process : function(){
	 var r = confirm("Are you sure you want to cancel the import?");
		if (r == true) {
		$.post(mw.settings.api_url+'Piksera/Utils/Import/batch_process', {cancel:true} ,
			function(msg) {
