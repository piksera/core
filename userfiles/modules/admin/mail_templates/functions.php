...
//autoload_add_namespace(__DIR__.'/src/', 'PikseraPackages\\Admin\\MailTemplates\\');
...
function save_mail_template($data)
{
    ...
    if (isset($data['id'])) {
        $findMailTemplate = PikseraPackages\Notification\Models\MailTemplate::where('id', $data['id'])->first();
    }
    if ($findMailTemplate == null) {
        $findMailTemplate = new PikseraPackages\Notification\Models\MailTemplate();
    }
...
function get_mail_templates($params = array())
{
    ...
    $getTemplates = PikseraPackages\Notification\Models\MailTemplate::all();
