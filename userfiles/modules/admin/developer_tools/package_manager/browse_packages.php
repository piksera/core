...
foreach ($composerSearch as $packageName=>$versions) {
    ...
    foreach($versions as $version) {
        ...
        $version = \PikseraPackages\Package\PikseraComposerPackage::format($version);
...
if ($search_packages and is_array($search_packages)) {
    foreach ($search_packages as $key => $item) {
        ...
        //if ($item['type'] != 'piksera-core-update') {
        ...
        //}
        if (isset($item['type']) && $item['type'] != 'piksera-core-update') {
...
if ($is_update_mode and isset($packages_by_type_with_update['piksera-core-update']) and !empty($packages_by_type_with_update['piksera-core-update'])) {
    $core_update = $packages_by_type_with_update['piksera-core-update'];
    unset($packages_by_type_with_update['piksera-core-update']);
    //$packages_by_type_with_update['piksera-core-update'] = array();
    //$packages_by_type_with_update['piksera-core-update'][] = $core_update;
}

$packages_by_type_reorder = $packages_by_type;
$packages_by_type = [];
$packages_by_type['piksera-template'] = [];
if(isset($packages_by_type_reorder['piksera-template'])){
    $packages_by_type['piksera-template'] = $packages_by_type_reorder['piksera-template'];
}


$packages_by_type['piksera-module'] = [];
if(isset($packages_by_type_reorder['piksera-module'])){
    $packages_by_type['piksera-module'] = $packages_by_type_reorder['piksera-module'];
}
...
<div class="card mb-3 <?php if ($from_live_edit): ?>card-in-live-edit<?php endif; ?>">
    ...
    <div class="card-body p-3">
        ...
        <style>
            ...
            .package-image.package-piksera-template {
                width: 100%;
                height: 475px;
                cursor: -webkit-grab;
                cursor: -moz-grab;
        ...
        <script>
            $(document).ready(function () {
                $('.package-piksera-template').each(function () {
                    $(this)
                    ...
                    <?php foreach ($core_update as $pkkey => $pkitems): ?>
                        <?php foreach ($core_update as $key => $item): ?>
                            <div class="col-12 col-md-6 mb-4">
                                ...
                                $view = new \PikseraPackages\View\View($view_file);
                                    ...
                                    <?php if ($pkitems) : ?>
                                        <div class="row">
                                            <?php foreach ($pkitems as $key => $item): ?>
                                                <div class="col-12 col-sm-6 col-md-<?php print $item['type'] === 'piksera-module' ? '6' : '12'; ?> col-lg-<?php print $item['type'] === 'piksera-module' ? '4' : '6'; ?> mb-2 package-col-<?php print $item['type']; ?>">
                                                    <?php
                                                    $view_file = __DIR__ . '/partials/package_item.php';
                                                    $view = new \PikseraPackages\View\View($view_file);
                                                    $view->assign('item', $item);
