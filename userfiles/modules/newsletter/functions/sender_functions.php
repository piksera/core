...
function newsletter_get_sender($data) {
    ...
    $query = \PikseraPackages\Modules\Newsletter\Models\NewsletterSenderAccount::query();
    if (isset($data['id'])) {
...
function newsletter_get_senders() {
    ...
    $get = \PikseraPackages\Modules\Newsletter\Models\NewsletterSenderAccount::all();
    if ($get) {
        return $get->toArray();
    }
    return [];
