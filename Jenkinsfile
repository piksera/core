...
def getKubeLabel = { Integer index, String gitBranch, String buildNumber ->
    return "piksera-unit-tests-${index}-${gitBranch}-${buildNumber}"
}
...
def components = ['piksera-modules/multilanguage', 'PYovchevski/MW-Module-Videos-Playlist']


pipeline {
  //environment {
    //registryCredential = 'piksera-dockerhub'
    //DOCKER_BUILDKIT = "1"
  //}
  agent {
    node {
    ...
    stage('UI Testing') {
      parallel {
        stage('PHP 7.3 Nginx-fpm') {
          ...
          steps {
            sh 'composer install -o --no-progress'
            sh 'php artisan piksera:install admin@admin.com admin password localhost mysql root mysql -p site_ -t dream -d 1'
            ...
            container('cypress') {
                checkout([
                           ...
                           submoduleCfg: [],
                           userRemoteConfigs: [[credentialsId: 'github-credentials-brightside', url: 'https://github.com/piksera-dev/cypress-tests.git']]
                       ])
