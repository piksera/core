...
class MigrateOldFormsData extends Migration
{
    ...
    public function up()
    {
        $getFormsData = \PikseraPackages\Form\Models\FormData::all();

        if ($getFormsData and $getFormsData->count() >0) {
            foreach ($getFormsData as $formData) {
                $findFormDataValues = \PikseraPackages\Form\Models\FormDataValue::where('form_data_id', $formData->id)->first();
                if ($findFormDataValues == null) {
                    ...
                    if (!empty($formDataFormValues)) {
                        foreach ($formDataFormValues as $dataKey=>$dataValue) {
                            ...
                            $formDataValue = new \PikseraPackages\Form\Models\FormDataValue();
                            ...
                            if ($dataKey) {
                                $findCf = (new \PikseraPackages\CustomField\Models\CustomField())->where('name_key',$fieldKey)->first();
