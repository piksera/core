...
namespace PikseraPackages\User;
...
class UserManager
{
    ...
    public function codeLogin()
    {
        ...
        $verifyUrl = $whmcsSettings['whmcs_url'] . '/index.php?m=piksera_addon&function=verify_login_code&code=' . $code . '&domain=' . $domain;
    ...
    public function picture($user_id = false)
    {
        ...
        return modules_url() . 'piksera/api/libs/mw-ui/assets/img/no-user.svg';
    }
            ...
            if ($register_email_enabled == true) {
                ...
                if ($register_email_content != false and trim($register_email_subject) != '') {
                    ...
                    if (isset($to) and (filter_var($to, FILTER_VALIDATE_EMAIL))) {
                        ...
                        $twig = new \PikseraPackages\View\TwigView();
                        $parsed_twig_email = $twig->render($register_email_content, $data);

                        $sender = new \PikseraPackages\Utils\Mail\MailSender();
...
//                    Session::save();
    ...
    public function socialite_config($provider = false)
    {
        ...
        if (get_option('enable_user_piksera_registration', 'users') == 'y') {
            $svc = Config::get('services.piksera');
            if (!isset($svc['client_id'])) {
                Config::set('services.piksera.client_id', get_option('piksera_app_id', 'users'));
            }
            if (!isset($svc['client_secret'])) {
                Config::set('services.piksera.client_secret', get_option('piksera_app_secret', 'users'));
            }
            if (!isset($svc['redirect'])) {
                Config::set('services.piksera.redirect', $callback_url);
            }
    ...
    public function get_shipping_address()
    {
        ...
        if ($this->is_logged()) {
            $findCustomer = \PikseraPackages\Customer\Models\Customer::where('user_id', Auth::id())->first();
            if ($findCustomer) {
                $findAddressShipping = \PikseraPackages\Customer\Models\Address::where('type', 'shipping')->where('customer_id', $findCustomer->id)->first();
    ...
    private function __check_id_has_ability_to_edit_user($user_id)
    {
        ...
        $disable_edit_users = Config::get('piksera.users_disable_edit');
