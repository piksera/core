...
@endforeach
        ...
        <div class="mt-2">
            @if (! $this->enabled)
                <x-piksera-ui::button type="button" wire:click="enableTwoFactorAuthentication" wire:loading.attr="disabled">
                    <?php _e('Enable');?>
                </x-piksera-ui::button>
            @else
                @if ($showingRecoveryCodes)
                    <x-piksera-ui::secondary-button class="mr-3" wire:click="regenerateRecoveryCodes">
                        <?php _e('Regenerate Recovery Codes');?>
                    </x-piksera-ui::secondary-button>
                @else
                    <x-piksera-ui::secondary-button class="mr-3" wire:click="$toggle('showingRecoveryCodes')">
                        <?php _e('Show Recovery Codes');?>
                    </x-piksera-ui::secondary-button>
                @endif

                <x-piksera-ui::danger-button wire:click="disableTwoFactorAuthentication" wire:loading.attr="disabled">
                    <?php _e('Disable');?>
