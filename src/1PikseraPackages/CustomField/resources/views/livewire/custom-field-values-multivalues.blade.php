<div class="mt-3">
    <x-piksera-ui::label value="Values" />
    ...
    <div id="js-sortable-items-holder-{{$this->id}}">
        @foreach($customField->fieldValue as $fieldValue)
            <div class="d-flex gap-3 mt-3 js-sortable-item" sort-key="{{ $fieldValue->id }}" wire:key="custom-field-vals-{{ $this->id }}-{{ $fieldValue->id }}">
                ...
                <div class="w-full">
                    <x-piksera-ui::input class="mt-1 block w-full" wire:model.debounce="inputs.{{ $fieldValue->id }}" />
                ...
                @if (isset($customField->options['as_price_modifier']) && $customField->options['as_price_modifier'])
                    <div class="w-full" style="max-width: 120px;">
                        <x-piksera-ui::input type="number" class="mt-1 block w-full" wire:model.debounce="priceModifiers.{{ $fieldValue->id }}" placeholder="0" />
    ...
    <div class="mt-3">
        <x-piksera-ui::label for="as_price_modifier" value="Use a price modifier for value" />
        <x-piksera-ui::toggle id="as_price_modifier" class="mt-1 block w-full" wire:model="state.options.as_price_modifier" />
    </div>
