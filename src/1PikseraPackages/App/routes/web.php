...
/*
use PikseraPackages\Translation\Models\Translation;
...
Route::group([
    //'middleware' => \PikseraPackages\App\Http\Middleware\SessionlessMiddleware::class,
...
Route::group(['middleware' => 'api.static', 'namespace' => '\PikseraPackages\App\Http\Controllers'], function () {
    Route::any('/userfiles/{path}', ['uses' => '\PikseraPackages\App\Http\Controllers\ServeStaticFileContoller@serveFromUserfiles'])->where('path', '.*');
});
...
Route::post('/csrf-validate-token', function () {
    ...
    $request = request();
    $middleware = app()->make(\PikseraPackages\App\Http\Middleware\VerifyCsrfToken::class);
    ...
    \PikseraPackages\App\Http\Middleware\IsAjaxMiddleware::class,
    \PikseraPackages\App\Http\Middleware\EncryptCookies::class,
...
Route::group([
    ...
    Route::any('/module/', '\PikseraPackages\App\Http\Controllers\ApiController@module');
    Route::any('module/{all}', array('as' => 'module', 'uses' => '\PikseraPackages\App\Http\Controllers\ApiController@module'))->where('all', '.*');
});

Route::group(['middleware' => ['public.web'], 'namespace' => '\PikseraPackages\App\Http\Controllers'], function () {
...
Route::group(['middleware' => ['public.web', \PikseraPackages\App\Http\Middleware\SessionlessMiddleware::class], 'namespace' => '\PikseraPackages\App\Http\Controllers'], function () {
...
//    $custom_admin_url = \Config::get('piksera.admin_url');
//    $admin_url = 'admin';
//    if ($custom_admin_url) {
