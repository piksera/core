...
Route::name('api.')
    ...
    ->namespace('\PikseraPackages\Product\Http\Controllers\Api')
    ->group(function () {
        ...
        Route::post('product_variant/parent/{id}/options', function($parentId) {
            $findProduct = \PikseraPackages\Product\Models\Product::where('id', $parentId)->first();
            if ($findProduct != null) {
                 ...
                 foreach ($getProductVariants as $productVariant) {
                     ...
                     if ($getContentDataVariant->count() > 0) {
                         foreach ($getContentDataVariant as $contentDataVariant) {
                             $getCustomFieldValue = \PikseraPackages\CustomField\Models\CustomFieldValue::where('id', $contentDataVariant->custom_field_value_id)->first();
        ...
        Route::post('product_variant_save', function() {
            ...
            if (!empty($options)) {
                ...
                $findProduct = \PikseraPackages\Product\Models\Product::where('id', $productId)->first();
                if ($findProduct != null) {
