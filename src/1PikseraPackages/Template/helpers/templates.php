...
use PikseraPackages\MetaTags\Facades\FrontendMetaTags;
...
function get_template_colors_settings()
{
    ...
    if (!isset($getTemplateConfig)) {
        throw new \PikseraPackages\Template\Exceptions\MissingTemplateException('Template is not selected. Please do to admin panel and select a template.');

    } else if (!isset($getTemplateConfig['dir_name'])) {
        throw new \PikseraPackages\Template\Exceptions\MissingTemplateException('Template dir_name is not set');
    }
...
//{
//    $in = new \PikseraPackages\Template\Adapters\RenderHelpers\TemplateMetaTagsRenderer();
