...
if (!function_exists('run_translate_manager')) {
    function run_translate_manager()
    {
        ...
        if (is_lang_correct($currentLocale)) {
            $translate = new \PikseraPackages\Multilanguage\TranslateManager();
            $translate->run();
        }
    }
}
...
if (!function_exists('add_supported_language')) {
    ...
    {
        $findSupportedLocales = \PikseraPackages\Multilanguage\Models\MultilanguageSupportedLocales::where('locale', $locale)->first();
        if ($findSupportedLocales == null) {
            $findSupportedLocales = new \PikseraPackages\Multilanguage\Models\MultilanguageSupportedLocales();
        ...
        if (!empty($languages)) {
            foreach ($languages as &$language) {
                ...
                if (empty($language['display_name'])) {
                    $language['language'] = \PikseraPackages\Translation\LanguageHelper::getDisplayLanguage($language['locale']);
                }
            }
        }
...
if (!function_exists('get_geolocation_detailed')) {
    ...
    function get_geolocation_detailed()
    {
        ...
        if ($geolocationProvider == 'ipstack_com') {
            ...
            if ($defaultWebsiteLanguage !== $browserLanguage) {
                $ipInfo = mw()->http->url('http://ipinfo.pikseraapi.com/?ip=' . $ip)->get();
            }
...
if (!function_exists('multilanguage_get_all_links')) {
    ...
    {
        $generator = new \PikseraPackages\Multilanguage\MultilanguageLinksGenerator();
