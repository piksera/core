...
namespace PikseraPackages\Security\HtmlSanitizer;
...
class MwHtmlSanitizerConfig extends \Symfony\Component\HtmlSanitizer\HtmlSanitizerConfig
{
    public function allowStaticElements(): static
    {
        $elements = array_merge(
            array_keys(MwHtmlSanitizerReference::PS_ELEMENTS),
    ...
    public function allowSafeElements(): static
    {
        ...
        foreach (MwHtmlSanitizerReference::PS_ATTRIBUTES as $attribute => $isSafe) {
            if ($isSafe) {
                $attributes[] = $attribute;
            }
        }
...
//        }
//
//        foreach (MwHtmlSanitizerReference::PS_ELEMENTS as $element => $isSafe) {
//            if ($isSafe) {
//                $clone = $clone->allowElement($element, '*');
//            }
//        }
