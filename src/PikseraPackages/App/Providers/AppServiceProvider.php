...
namespace PikseraPackages\App\Providers;
...
use PikseraPackages\Admin\Providers\AdminRouteServiceProvider;
use PikseraPackages\App\Console\Commands\ServeCodeCoverageTestCommand;
use PikseraPackages\App\Console\Commands\ServeTestCommand;
use PikseraPackages\App\Http\Middleware\AuthenticateSessionForUser;
use PikseraPackages\App\Http\Middleware\TrimStrings;
use PikseraPackages\App\Utils\Parser;
use PikseraPackages\Config\ConfigSave;
use PikseraPackages\Config\ConfigSaveServiceProvider;
...
use PikseraPackages\Utils\System\ClassLoader;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    */
    public $laravel_providers = [
        ...
        \PikseraPackages\Pagination\PaginationServiceProvider::class,
    ...
    */
    ...
    public function register()
    {
        ...
        if (config::get('piksera.force_https') && !is_cli()) {
...
//                }
                ...
                if(is_dir(config_path($domain)) and is_file(config_path($domain) . '/piksera.php')) {
    ...
    protected function registerSingletonProviders()
    {
        ...
        foreach ($providers as $alias => $class) {
            $this->app->singleton($alias, function ($app) use ($class) {
                $class = 'PikseraPackages\\App\Managers\\' . $class;
...
//        }
        ...
        if (is_cli()) {
            $this->commands('PikseraPackages\Console\Commands\ResetCommand');
        }
        ...
        //    load_all_functions_files_for_modules();
            ...
            if (is_cli()) {
                ...
                $this->commands('PikseraPackages\Option\Console\Commands\OptionCommand');
                $this->commands('PikseraPackages\Console\Commands\UpdateCommand');
                $this->commands('PikseraPackages\Console\Commands\ModuleCommand');
                $this->commands('PikseraPackages\Console\Commands\ReloadDatabaseCommand');
                $this->commands('PikseraPackages\Console\Commands\PackageInstallCommand');
                $this->commands('PikseraPackages\Console\Commands\AddLicenseKeyCommand');

            }
        } else {
            // Otherwise register the install command
            $this->commands('PikseraPackages\Install\Console\Commands\InstallCommand');
        }
        ...
        // >>> MW Kernel add
         $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware( \PikseraPackages\App\Http\Middleware\TrustProxies::class);
    //    $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware( \Illuminate\Http\Middleware\TrustProxies::class);
        ...
        $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware(\PikseraPackages\App\Http\Middleware\CheckForMaintenanceMode::class);
        ...
        $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware(\PikseraPackages\App\Http\Middleware\TrimStrings::class);
       // $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware(\Illuminate\Session\Middleware\StartSession::class);
        $this->app->make('Illuminate\Contracts\Http\Kernel')->prependMiddleware(\PikseraPackages\App\Http\Middleware\StartSessionExtended::class);
        ...
        $router->pushMiddlewareToGroup('web', \PikseraPackages\App\Http\Middleware\EncryptCookies::class);
        ...
        $router->pushMiddlewareToGroup('web',  \PikseraPackages\App\Http\Middleware\VerifyCsrfToken::class);
        $router->pushMiddlewareToGroup('web',  \Illuminate\Routing\Middleware\SubstituteBindings::class);

        $router->aliasMiddleware('auth', \PikseraPackages\App\Http\Middleware\Authenticate::class);
        ...
        $router->aliasMiddleware('guest', \PikseraPackages\App\Http\Middleware\RedirectIfAuthenticated::class);
        ...
        $router->aliasMiddleware('throttle', \PikseraPackages\App\Http\Middleware\ThrottleExternalRequests::class);
        $router->aliasMiddleware('verified', \Illuminate\Auth\Middleware\EnsureEmailIsVerified::class);
        $router->aliasMiddleware('xss', \PikseraPackages\App\Http\Middleware\XSS::class);
        $router->aliasMiddleware('remove_html', \PikseraPackages\App\Http\Middleware\RemoveHtml::class);
        $router->aliasMiddleware('admin', \PikseraPackages\Admin\Http\Middleware\Admin::class);
        $router->aliasMiddleware('api_auth', \PikseraPackages\App\Http\Middleware\ApiAuth::class);
        $router->aliasMiddleware('allowed_ips', \PikseraPackages\App\Http\Middleware\AllowedIps::class);
        ...
        $router->middlewareGroup('api.static',[
            \PikseraPackages\App\Http\Middleware\SessionlessMiddleware::class,
...
//
//        $cached = base_path('bootstrap/cache/piksera_packages.json');
