...
class UpdateManager
{
    ...
    private $remote_api_url = 'https://update.pikseraapi.com/';
    private $remote_url = 'https://update.pikseraapi.com/';
    ...
    {
        return new \PikseraPackages\Utils\Http\Http();
    }
    ...
    public function marketplace_link($params = false)
    {
        ...
        } elseif ($this->app->make('config')->get('piksera.marketplace_provider_id')) {
            $params['marketplace_provider_id'] = $this->app->make('config')->get('piksera.marketplace_provider_id');
        }
        ...
        if (mw_is_installed()) {
            ...
            if (!defined('TEMPLATE_DIR')) {
                ...
                if (!$the_active_site_template) {
                    $the_active_site_template = Config::get('piksera.install_default_template');
                }
                if ($the_active_site_template) {
            ...
            if ($version != false) {
                Config::set('piksera.version', $version);
                Config::set('piksera.updated_at', @date("Y-m-d H:i:s"));
                Config::save('piksera');
            }
        }
        return true;
    ...
    public function send_anonymous_server_data($params = false)
    {
        ...
        $params['function_name'] = 'send_lang_form_to_piksera';
