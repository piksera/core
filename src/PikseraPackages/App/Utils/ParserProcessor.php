...
namespace PikseraPackages\App\Utils;


use Doctrine\DBAL\Connection;
use PikseraPackages\App\Utils\ParserHelpers\ParserLayoutItem;
use PikseraPackages\App\Utils\ParserHelpers\ParserModuleItem;
use PikseraPackages\App\Utils\ParserHelpers\ParserModuleItemCollection;
...
//        }
        ...
        if (!$should_parse_only_vars) {
            ...
            $layout = str_replace('<piksera module=', '<module data-type=', $layout);
            $layout = str_replace('</piksera>', '', $layout);
...
//                                }
                                    ...
                                    if ($plain_modules != false) {
                                        if (!defined('PS_PLAIN_MODULES')) {
                                            define('PS_PLAIN_MODULES', true);
                                        }
                                    }
                                    foreach ($attrs as $nn => $nv) {
...
//        }
     //   $layout = $this->_replace_tags_with_placeholders_back($layout);
        ...
        $layout = str_replace('{PS_SITE_URL}', app()->url_manager->site(), $layout);
