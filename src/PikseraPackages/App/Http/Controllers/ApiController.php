...
use PikseraPackages\App\Http\Middleware\ApiAuth;
use PikseraPackages\App\Http\Middleware\SameSiteRefererMiddleware;
use PikseraPackages\App\Managers\Helpers\VerifyCsrfTokenHelper;
...
class ApiController  extends FrontendController
{
    ...
    public function api_html()
    {
        if (!defined('PS_API_HTML_OUTPUT')) {
            define('PS_API_HTML_OUTPUT', true);
        }
        return $this->api();
    }
    ...
    public function api($api_function = false, $params = false)
    {
        ...
        $mod_api_class_native_system = normalize_path(dirname(PS_PATH) . DS . $mod_api_class, false) . '.php';
        ...
        if (!defined('PS_API_FUNCTION_CALL')) {
            define('PS_API_FUNCTION_CALL', $api_function);
        }
    ...
    public function module()
    {
        ...
        if (!defined('PS_NO_SESSION')) {
            ...
            if (!mw()->user_manager->session_id() and $is_ajax == false and !defined('PS_SESS_STARTED')) {
                define('PS_SESS_STARTED', true);
                //session_start();
            }
        ...
        if (isset($from_url) and $from_url != false) {
            ...
            if (stristr($from_url, admin_url()) && !defined('PS_BACKEND')) {
        ...
        if (isset($data['ondrop'])) {
            if (!defined('PS_MODULE_ONDROP')) {
                define('PS_MODULE_ONDROP', true);
            }
