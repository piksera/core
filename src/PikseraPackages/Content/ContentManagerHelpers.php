...
use PikseraPackages\Content\Models\ContentRelated;
...
class ContentManagerHelpers extends ContentManagerCrud
{
    public function add_content_to_menu($content_id, $menu_id = false)
    {
        ...
        if (defined('PS_API_CALL') and $id == false) {
            return;
        }
        $new_item = false;
        $id = $this->app->user_manager->is_admin();
    ...
    public function delete($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $del_ids[] = $c_id;
            if ($to_trash == false) {
                \PikseraPackages\Content\Models\Content::where('id', $c_id)->first()->delete();
            }
        }
...
//dd($del_ids);
    ...
    public function reset_modules_settings($modules_ids)
    {
        ...
        if (is_array($modules_ids) and !empty($modules_ids)) {
            foreach ($modules_ids as $modules_id) {
                if ($modules_id) {
                    \PikseraPackages\Option\Models\Option::where('option_group', '=', $modules_id)->delete();
                    \PikseraPackages\Media\Models\Media::where('rel_type', '=', 'modules')->where('rel_id', '=', $modules_id)->delete();
    ...
    {
        if (defined('PS_NO_DEFAULT_CONTENT')) {
            return true;
        }
