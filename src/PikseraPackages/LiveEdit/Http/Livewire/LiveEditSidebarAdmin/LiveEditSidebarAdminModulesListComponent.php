...
namespace PikseraPackages\LiveEdit\Http\Livewire\LiveEditSidebarAdmin;
...
{
    public string $view = 'piksera-live-edit::sidebar-admin.sidebar-admin-modules-list';
    ...
    private function buildRecursiveTree($recursiveArray)
    {
        ...
        $mwMenu = new \PikseraPackages\Admin\MenuBuilder\Menu('SIDEBAR_ADMIN_MODULES_TREE', $options);
    ...
    private function addChildItems($parentMenu, $items)
    {
        foreach ($items as $item) {
            ...
            $content = view('piksera-live-edit::sidebar-admin.partials.modules-tree-link-item', ['item' => $item])->render();
            $itemOptions = [
                //  'uri' => $uri,
                'label' => $content,
