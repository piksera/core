...
use PikseraPackages\Repository\PikseraQuery;
    ...
    public function getMenus($params)
    {
       // dump($params);
        ...
        return $this->cacheCallback(__FUNCTION__, func_get_args(), function () use ($params) {
            ...
            $menus = PikseraQuery::execute(Menu::query(), $params);

            if (!empty($menus)) {
                return $menus;
            } else {
                if (!defined('PS_MENU_IS_ALREADY_MADE_ONCE')) {
                    ...
                    define('PS_MENU_IS_ALREADY_MADE_ONCE', true);
                }
            }
            if (!empty($menus)) {
                return $menus;
