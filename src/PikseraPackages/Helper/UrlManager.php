...
if (!defined('PS_ROOTPATH')) {
    if (function_exists('base_path')) {
        define('PS_ROOTPATH', base_path() . DIRECTORY_SEPARATOR);
    } else {
        define('PS_ROOTPATH', DIRECTORY_SEPARATOR);
    }
}
...
class UrlManager
{
    ...
    public function to_path($path)
    {
        ...
        $path = str_ireplace($this->site_url(), PS_ROOTPATH, $path);
        $path = str_replace('\\', '/', $path);
        $path = str_replace('//', '/', $path);
    ...
    {
        return $this->param($param = '__PS_GET_ALL_PARAMS__', $skip_ajax);
    }
    ...
    public function param($param, $skip_ajax = false, $force_url = false)
    {
        ...
        foreach ($segs as $segment) {
            ...
            if ($param == '__PS_GET_ALL_PARAMS__') {
...
//        // Strip out anything we haven't been able to convert
    ...
    public function download($requestUrl, $post_params = false, $save_to_file = false)
    {
        ...
        $opts = array('http' => array('method' => 'POST', 'header' => 'User-Agent: Piksera/' . PS_V . "\r\n" . 'Content-type: application/x-www-form-urlencoded' . "\r\n" . 'Referer: ' . $ref . "\r\n", 'content' => $postdata));
        ...
        if (function_exists('curl_init')) {
            ...
            curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/4.0 (compatible; MSIE 5.01; Piksera ' . PS_V . ';)');
