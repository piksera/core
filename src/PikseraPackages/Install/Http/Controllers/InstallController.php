...
namespace PikseraPackages\Install\Http\Controllers;
...
class InstallController extends Controller
{
   ...
   public function selectTemplateView()
    ...
    public function index($input = null)
    {
        ...
        $allowed_configs = array('database', 'piksera');
        ...
        if (isset($input['make_install'])) {
            ...
            if (isset($input['default_template']) and $input['default_template'] != false) {
                Config::set('piksera.install_default_template', $input['default_template']);
            }
            ...
            if (!isset($input['developer_mode'])) {
                Config::set('piksera.compile_assets', 1);
                Config::set('piksera.disable_model_cache', 0);
            }
            if (isset($input['clean_pre_configured'])) {
                Config::set('piksera.pre_configured', null);
                Config::set('piksera.pre_configured_input', null);
            }

            if (isset($input['admin_url'])) {
                Config::set('piksera.admin_url', $input['admin_url']);
            }
            if (!is_cli()) {
            ...
            if (isset($input['site_lang'])) {
                ...
                Config::set('piksera.site_lang', $input['site_lang']);
            }
            ...
            if ($config_only) {
                Config::set('piksera.pre_configured', 1);
                Config::set('piksera.pre_configured_input', $input);
            } else {
                ...
                if (!$install_step or $install_step == 7) {
                    if (isset($input['site_lang'])) {
                        ...
                        $selected_template = Config::get('piksera.install_default_template');
...
//                        TranslationPackageInstallHelper::installLanguage($input['site_lang']);
                ...
                if (isset($input['admin_password']) && strlen($input['admin_password'])) {
                    if (isset($input['admin_email']) or isset($input['admin_username'])) {
                        ...
                        Config::set('piksera.has_admin', 1);
                    }
                }

                $this->log('Saving ready config');

                Config::set('piksera.is_installed', 1);
            ...
            if (Config::get('piksera.has_admin') and !is_cli() and isset($admin_user_id)) {
        ...
        if (Config::get('piksera.pre_configured')) {
            $viewData['pre_configured'] = Config::get('piksera.pre_configured');
            if (Config::get('piksera.pre_configured_input') and is_array(Config::get('piksera.pre_configured_input'))) {
                $viewData['config'] = array_merge($viewData['config'], Config::get('piksera.pre_configured_input'));
            }
        }
        $layout->set($viewData);
    ...
    private function reportInstall($email, $sendMail = false)
    {
        ...
        $um = new \PikseraPackages\App\Managers\UpdateManager(app());
        ...
        try {
            $http->url('https://installreport.services.pikseraapi.com')->set_timeout(10)->post($postData);
        } catch (\Exception $e) {
    ...
    private function _getMarketTemplatesForInstallScreen()
    {
        ...
        if ($results and is_array($results)) {
            foreach ($results as $k => $result) {
                ...
                if ($latestVersion['type'] !== 'piksera-template') {
                    continue;
                }/*
