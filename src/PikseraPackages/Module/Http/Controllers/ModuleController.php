...
namespace PikseraPackages\Module\Http\Controllers;
...
class ModuleController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$is_installed) {
            App::abort(403, 'Unauthorized action. Piksera MUST be installed to use modules.');
        }
        ...
        if ($mod_n == 'layout') {
            if (isset($data['template'])) {
                ...
                if (is_file($possible_layout)) {
                    $l = new \PikseraPackages\View\View($possible_layout);
                    $layout = $l->__toString();
...
//dd($_REQUEST);
        ...
        if ($embed != false) {
            ...
            $l = new \PikseraPackages\View\View($p_index);
            $layout = $l->__toString();
...
//        }
       ...
       // $params = $_REQUEST;
        ...
        if (isset($_REQUEST['plain'])) {
            if (is_file($p)) {
                $p = new \PikseraPackages\View\View($p);
                $p->params = $params;
                $layout = $p->__toString();
               // echo $layout;
                return response($layout);
        ...
        if (!$standalone_edit && isset($page['render_file'])) {
                $l = new \PikseraPackages\View\View($page['render_file']);
                $l->page_id = PAGE_ID;
...
//                }
        ...
        //$v = new \PikseraPackages\View\View(PS_ADMIN_VIEWS_DIR . '404.php');
        //echo $v;
    }
